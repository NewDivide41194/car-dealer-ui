...
import { fetchCars, setQuery } from "../../features/carsSlice";
...
const SearchInput = () => {
    ...
    const handleSearch = (searchTerm: string) => {
        ...
        dispatch(fetchCars({ page: 1, query: searchTerm }));
    };
            ...
            <div className="flex">
                ...
                <div id="dropdown" className="z-10 hidden bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700">
                    <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="dropdown-button">
                        {categories.map((v, k) => <li key={k}>
                            <button type="button" className="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{v.name}</button>
                        </li>)
                        }
                    </ul>
                </div>
